...
// configuración SMTP
    protected $smtp_host        = 'smtp.mailtrap.io';
...
//---------------------------- constructor --------------------------------
     ...
     */
    public function __construct( $sender_mail ,  $primera_linea = 'Información de contacto' ){
...
//---------------------------- definir un SMTP diferente  --------------------------------
    ...
    public function setSMTP($smtp_config = null)
    {
        ...
        if (!is_null($smtp_config)) {
            ...
            $this->smtp_mail        = array_key_exists('mail', $smtp_config)            ? $smtp_config['mail']        : $this->sender_mail;
...
//----------------------------funciones para el envió del correo --------------------------------
    ...
    */
    ...
    public function CltvoMailsSender($mailContentForProvider){ // envía correo de registro y notificación
        ...
        if (!$this->esp_smtp) {
            ...
            $mailForProvider = $this->send($subjectMailToProvider, $mailContentForProvider, $headFromForProvider, $this->sender_mail, get_bloginfo('name'));
            ...
            try{
                $provider_mail = $this->CltvoPhpMailer($this->sender_mail, $subjectMailToProvider, $mailContentForProvider);
            }catch(Error $e){
                var_dump($e);
                die;
            }

            $provider_mail->addReplyTo($this->suscriber_mail);
    ...
    */
    ...
    private function mail_template($greeting = "", $level = "", $introLines = array(), $actionText = null, $actionUrl = null, $outroLines = array(), $dynamic = false)
    {
        ...
        $template .= '<a href="'.get_site_url().'" target="_blank" alt="" title="'.get_bloginfo('name').'" style="display: inline-block;margin: 25px auto;"><img src="'.get_bloginfo('template_url').'/images/logo.png" width="156" /></a><br>';
